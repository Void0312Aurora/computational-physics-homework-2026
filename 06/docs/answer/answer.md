...
$$
D_h^{(r)}f_i=\frac{1}{h^r}\sum_{k\in S}c_k f_{i+k},
$$

其中 $S$ 为模板节点集合，$c_k$ 为待定系数。对每个 $f_{i+k}$ 在 $x_i$ 处做 Taylor 展开，

$$
f_{i+k}
=f_i+khf_i'+\frac{k^2h^2}{2!}f_i''
+\frac{k^3h^3}{3!}f_i^{(3)}
+\frac{k^4h^4}{4!}f_i^{(4)}+\cdots,
$$
...
D_h^{(r)}f_i
=\frac{1}{h^r}\left[
\left(\sum_k c_k\right)f_i
+h\left(\sum_k kc_k\right)f_i'
+\frac{h^2}{2!}\left(\sum_k k^2c_k\right)f_i''
+\cdots
\right].
$$

因此只要令低阶项全部消失，并让 $r$ 阶项的系数等于 $1$，就得到矩条件

$$
\sum_{k\in S}k^m c_k=0\quad (m\ne r),\qquad
\sum_{k\in S}k^r c_k=r!.
$$
