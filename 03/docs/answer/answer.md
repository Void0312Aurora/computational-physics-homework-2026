...
half precision 在 $1$ 附近的相邻间隔为 $2^{-10}$，最近舍入的半间隔为 $2^{-11}$。因此当 $t=2^{-11}=0.00048828125$ 时，$1+t$ 处在舍入边界并被舍入回 $1$，进而使 $(1+t)^2-1$ 变为 $0$，直接表达式的相对误差跃升到 1.0000E+0。另一方面，$2$ 附近的 half 间隔是 $2^{-9}$，其半间隔是 $2^{-10}$；所以 $2+t$ 的改写形式在 $t=2^{-10}$ 开始被量化为 $2$，相对误差也会出现台阶。由此可见，图中的跃变是浮点量化阈值造成的确定性现象，不是随机噪声或绘图错误。
...
由前两问可直接得到结论：本机半精度的机器精度为 $2^{-10}=0.0009765625$。在算例

$$
\frac{(1+t)^2-1}{t}
$$

中，当 $t=2^{-11}=0.00048828125$ 时，半精度直接计算结果为 $0$，而精确值仍为 $2.00048828125$，相对误差达到 1.0000E+0。也就是说，half precision 下的 roundoff error 可以严重到把本应为 $2$ 量级的结果直接压成 $0$。

该结论直接由 Problem 3(1) 和 Problem 3(2) 的结果归纳得到。核心事实是 half precision 在 $1$ 附近只有约 $2^{-10}$ 的相邻分辨率，因此 $1+t$ 对小扰动并不连续响应；当 $t$ 落到半间隔 $2^{-11}$ 附近时，直接表达式会先把 $1+t$ 舍入回 $1$，再把整个分子压成 $0$。改写为 $2+t$ 后仍有 half 量化误差，但不会把结果从 $2$ 量级直接压到 $0$。
